...
namespace BWAPI
{
  ...
  Unitset Unitset::getUnitsInRadius(int radius, const UnitFilter &pred) const
  {
    ...
    return BroodwarImpl.getUnitsInRadius(this->getPosition(), radius, pred);
  }
  Unit Unitset::getClosestUnit(const UnitFilter &pred, int radius) const
  {
    ...
    return BroodwarImpl.getClosestUnit(this->getPosition(), pred, radius);
  }

  bool Unitset::issueCommand(UnitCommand command) const
  {
    return BroodwarImpl.issueCommand(*this, command);
  }
  //--------------------------------------------- ATTACK MOVE ------------------------------------------------
