...
namespace BWAPI
{
  ...
  void AssignBuildableLocations(PlacementReserve &reserve, UnitType type, TilePosition desiredPosition)
  {
    ...
    reserve.iterate( [&](PlacementReserve *pr, int x, int y)
                      { 
                        if ( (!hasAddon || BroodwarImpl.canBuildHere(start+TilePosition(x+4,y+1), UnitTypes::Terran_Missile_Turret) ) &&
                          BroodwarImpl.canBuildHere(start+TilePosition(x,y), type) )
                        {
                          pr->setValue(x, y, 1);
                        }
                      });
  ...
  void RemoveDisconnected(PlacementReserve &reserve, TilePosition desiredPosition)
  {
    ...
    reserve.iterate( [&](PlacementReserve *pr, int x, int y)
                      { 
                        if ( !BroodwarImpl.hasPath(Position(desiredPosition), Position(start + TilePosition(x,y)) ) )
                          pr->setValue(x, y, 0);
                      });
  }
  ...
  void ReserveUnbuildable(PlacementReserve &reserve, UnitType type, TilePosition desiredPosition)
  {
    ...
    reserve.iterate( [&](PlacementReserve *, int x, int y)
                      { 
                        if ( !BroodwarImpl.isBuildable( start + TilePosition(x,y), type != UnitTypes::Special_Start_Location ) )
  ...
  void ReserveGroundHeight(PlacementReserve &reserve, TilePosition desiredPosition)
  {
    ...
    reserve.backup();
    int targetHeight = BroodwarImpl.getGroundHeight(desiredPosition);
    reserve.iterate( [&](PlacementReserve *pr, int x, int y)
                      { 
                        if ( BroodwarImpl.getGroundHeight( start + TilePosition(x,y) ) != targetHeight )
                          pr->setValue(x, y, 0);
                      });
  ...
  void ReserveExistingAddonPlacement(PlacementReserve &reserve, TilePosition desiredPosition)
  {
    ...
    reserve.backup();
    Unitset myUnits = BroodwarImpl.self()->getUnits();
  ...
  void ReserveAllStructures(PlacementReserve &reserve, UnitType type, TilePosition desiredPosition)
  {
    ...
    {
      Unitset resources = BroodwarImpl.getNeutralUnits();
    ...
    for ( int passCount = 0; passCount < (pTargRegion ? 2 : 1); ++passCount )
    {
      for ( int y = 0; y < MAX_RANGE; ++y )
        for ( int x = 0; x < MAX_RANGE; ++x )
        {
          ...
          int currentDistance = desiredPosition.getApproxDistance(currentPosition);//BroodwarImpl.getGroundDistance( desiredPosition, currentPosition );
