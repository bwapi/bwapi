...
{
  /**
   * Utility for work with property files. Dictionary represents content of set
   * of property files with constant(length of word) acces to it's values.
   * Dictionary consists of two different data structures, the first one is hash
   * table of all values, the second one is list of files (DictionaryFile)
   * containing list of lines (DictionaryLine). The first strcture enables fast
   * acces property values, while the second one enables to change some property
   * values and save these changes back to it's dictionary files while position
   * of every property value will be unchanged same as all comemnts, this is
   * practical mainly for configuration property files.<br> Every value of the
   * dicionary is represnetd by class Sentence, and the same sentence is
   * referenced both from DictionaryLine and the hash table.
   * <br><br>
   * property file syntax: <br> key=value, parameter 1 is {1}, parameter 3 is
   * {3}, value of hello is {hello}
   * @see Sentence
   */
  class Dictionary
   {
