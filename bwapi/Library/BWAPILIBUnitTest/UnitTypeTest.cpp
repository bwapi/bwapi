...
TEST_P(UnitTypeFixture, airWeapon) {
	...
	if (u == UnitTypes::Terran_Goliath) {
		ASSERT_EQ(u.airWeapon().getID(), 8);
	}
	else if (u == UnitTypes::Hero_Alan_Schezar) {
		ASSERT_EQ(u.airWeapon().getID(), 10);
	}
	else {
...
TEST_P(UnitTypeFixture, groundWeapon) {
	UnitType u = GetParam();
	if (u == UnitTypes::Terran_Goliath) {
		ASSERT_EQ(u.groundWeapon().getID(), 7);
	}
	else if (u == UnitTypes::Hero_Alan_Schezar) {
		ASSERT_EQ(u.groundWeapon().getID(), 9);
	}
	else if (u == UnitTypes::Terran_Siege_Tank_Tank_Mode) {
		ASSERT_EQ(u.groundWeapon().getID(), 11);
	}
	else if (u == UnitTypes::Terran_Siege_Tank_Siege_Mode) {
		ASSERT_EQ(u.groundWeapon().getID(), 27);
	}
	else if (u == UnitTypes::Hero_Edmund_Duke_Tank_Mode ) {
		ASSERT_EQ(u.groundWeapon().getID(), 12);
	}
	else if (u == UnitTypes::Hero_Edmund_Duke_Siege_Mode) {
		ASSERT_EQ(u.groundWeapon().getID(), 28);
	}
	else {
