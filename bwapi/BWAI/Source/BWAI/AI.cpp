...
namespace BWAI
{
  ...
  bool AI::betterMinralPatch(BWAI::TaskGather* task1, BWAI::TaskGather* task2, Unit* optimiseFor)
  {
    ...
    if (task1->executors.size() > task2->executors.size())
      return false;
    if (optimiseFor != NULL && task1->getExpansion()->gatherCenter != task2->getExpansion()->gatherCenter)
    {
  ...
  bool AI::betterWorkerToFree(Unit* worker1, Unit* worker2, const BWAPI::Position& buildingPosition)
  {
    ...
    {
      if (worker1->getTask()->getType() == BWAI::TaskType::Gather && worker2->getTask()->getType() != BWAI::TaskType::Gather)
        return true;
      if (worker1->getTask()->getType() != BWAI::TaskType::Gather && worker2->getTask()->getType() == BWAI::TaskType::Gather)
        return false;
      if (worker1->getTask()->getType() == BWAI::TaskType::Gather)
        if (((TaskGather*)worker1->getTask())->getExpansion() == ((TaskGather*)worker1->getTask())->getExpansion() &&
            ((TaskGather*)worker1->getTask())->getExpansion() != NULL)
        {
          ...
          if (worker2->getOrder() == BWAPI::Orders::ReturnMinerals && worker1->getOrder() == BWAPI::Orders::MoveToMinerals)
            return true;
          if (worker1->getOrder() == BWAPI::Orders::ReturnMinerals && worker2->getOrder() == BWAPI::Orders::MoveToMinerals)
            return false;
  ...
  void AI::onFrame(void)
  {
    ...
    */
    ...
    try
    {
      if (BWAPI::Broodwar->getFrameCount() < 2 || !this->player)
        return;
  ...
  anotherStep:
    ...
    for each (TaskGather* i in this->activeMinerals)
    {
      if (best->executors.size() > i->executors.size())
        best = i;
      else if (worst->executors.size() < i->executors.size())
        worst = i;
    }
    ...
    for each (Unit* i in this->units)
    {
      if (!(i->isCompleted() &&
            (i->getType() == BWAPI::UnitTypes::Terran_Command_Center ||
             ...
             i->getType() == BWAPI::UnitTypes::Zerg_Hatchery) && i->getOwner() == player))
        continue;
