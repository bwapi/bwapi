...
void CancelConstructionTest::update()
{
  ...
  if (building!=NULL)
  {
    ...
    {
      if (thisFrame==startFrame+Broodwar->getLatencyFrames()+20)
      {
      ...
      if (thisFrame>=startFrame+Broodwar->getLatencyFrames()+20 && thisFrame<startFrame+Broodwar->getLatencyFrames()+30)
      {
        FAILTEST(builder->getBuildType()==UnitTypes::None);
        FAILTEST(builder->isConstructing()==false);
        FAILTEST(building->isBeingConstructed()==false);
    ...
    if (thisFrame==startFrame+Broodwar->getLatencyFrames()+50)
    {
      if (unitType.isAddon())
        builder->cancelAddon();
      else
    ...
    {
      FAILTEST(builder->getBuildType()==UnitTypes::None);
      BWAssertF(builder->isConstructing()==false,{log("%d, %s: %s,%s",thisFrame-(startFrame+Broodwar->getLatencyFrames()+50), builder->getType().c_str(), builder->getOrder().c_str(), builder->getSecondaryOrder().c_str());});
      BWAssertF(Broodwar->self()->minerals()==correctMineralCount,{log("%d: %d != %d",thisFrame-(startFrame+Broodwar->getLatencyFrames()+50),Broodwar->self()->minerals(),correctMineralCount);fail=true;return;});
      BWAssertF(Broodwar->self()->gas()==correctGasCount,{log("%d: %d != %d, diff = %d",thisFrame-(startFrame+Broodwar->getLatencyFrames()+50), Broodwar->self()->gas(),correctGasCount,Broodwar->self()->gas()-correctGasCount);fail=true;return;});
      BWAssertF(Broodwar->self()->supplyUsed()==correctSupplyUsedCount,{log("%d: %d != %d",thisFrame-(startFrame+Broodwar->getLatencyFrames()+50),Broodwar->self()->supplyUsed(),correctSupplyUsedCount);fail=true;return;});
    }
    if (thisFrame>startFrame+Broodwar->getLatencyFrames()+55)
    {
    ...
    if (thisFrame>startFrame+Broodwar->getLatencyFrames()+70)
    {
      running = false;
    }
  }
