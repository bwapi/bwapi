...
namespace Render
{
  BW::Bitmap drawSurface;
  BW::Position screenPosition;
  ...
  void drawMinitileImageData(WORD wTileImageIdx, int targx, int targy)
  {
    ...
    for (int y = 0; y < 8; ++y)
      for (int x = 0; x < 8; ++x)
        drawSurface.plot(targx + x, targy + y, imageData.cdata[y][isFlipped ? 8 - x - 1 : x]);
  }
  ...
  void drawMegatileImageData(WORD tile, int targTileX, int targTileY)
  {
    ...
    BW::Position scrPixelPos(BW::TilePosition((short)targTileX, (short)targTileY));
    scrPixelPos -= screenPosition;
    ...
    if (isCreepTile)
    {
      ...
      if (creepID != 0)   // Render terrain GRP for creep edge (if exists) to terrain cache
        drawSurface.BlitGraphic(*BW::BWDATA::TerrainGraphics, creepID - 1, scrPixelPos.x, scrPixelPos.y);
    }
  }

  void drawMapTiles()
  {
    BW::TilePosition moveTo(screenPosition);
    BW::TilePosition mapMax = *BW::BWDATA::MapSize;
    BW::TilePosition windowTileSize(BW::Position(static_cast<short>(Window::instance.width()), static_cast<short>(Window::instance.height())));
    windowTileSize += BW::TilePosition(1, 1);

    for (int y = moveTo.y; y < std::min((short)(moveTo.y + windowTileSize.y), mapMax.y); ++y)
    {
      for (int x = moveTo.x; x < std::min((short)(moveTo.x + windowTileSize.x), mapMax.x); ++x)
      {
  ...
  void DrawGameUpdate()
  {
    drawSurface = Window::instance.lockSurface();
    BW::Bitmap *oldSurface = BW::BWDATA::CurrentDrawSurface;
    BW::BWDATA::CurrentDrawSurface = &drawSurface;
    screenPosition = BW::Position(static_cast<short>(*BW::BWDATA::ScreenX), static_cast<short>(*BW::BWDATA::ScreenY));

    drawMapTiles();

    updateImageDrawingData();
    drawSprites(); // bug: Nexus does not appear!   //BW::BWFXN_drawAllSprites();// sprites
    ...
    BW::BWDATA::CurrentDrawSurface = oldSurface;
    Window::instance.unlockSurface();
  }
