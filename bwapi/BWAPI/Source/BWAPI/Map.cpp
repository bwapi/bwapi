...
//public:
  ...
  void setBuildability()
  {
    for (unsigned int y = 0; y < BWAPI::getHeight(); y++)
      for (unsigned int x = 0; x < BWAPI::getWidth(); x++)
        this->buildability[x][y] = !((BW::TileSet::getTileType(BWAPI::getTile(x, y))->buildability >> 4) & 0X8);
  }
  //-------------------------------------------- SET WALKABILITY ---------------------------------------------
  void setWalkability()
  {
    for (unsigned int y = 0; y < (u16)(BWAPI::getHeight()*4); y++)
      for (unsigned int x = 0; x < (u16)(BWAPI::getWidth()*4); x++)
  ...
  Util::BitMask<u16> getMiniTile(int x, int y) const
  {
    ...
    int mx = x % 4;
    int my = y % 4;
    BW::TileID tileID = BWAPI::getTile(tx, ty);
  ...
  int getMapHash()
  {
    ...
    for (unsigned int x = 0; x < BWAPI::getWidth(); x++)
      for (unsigned int y = 0; y < BWAPI::getHeight(); y++)
      {
        hashval += BW::TileSet::getTileType(BWAPI::getTile(x, y))->index;
        hashval *= x + y;
      }
