...
class Unit
 {
  public :
    ...
    int playerID;
    LPVOID address;
    LPVOID previousAddress, nextAddress;
    UnitType unitID;
    std::string unitName(void);
    short int getMoveX(void);
    void setMoveX(short int moveX);
    short int getMoveY(void);
    void setMoveY(short int moveY);
    int getHp(void);
    void setHp(int hp);
    int getAcceleration(void);
    void setAcceleration(int hp);
    int getShield(void);
    byte getHpFraction(void);
    byte getShieldFraction(void);
    Unit *getTarget(void);
    int temp;
    int orderTargetAddress;
    int targetAddress;
    void updateTarget(void);
    Unit *target;
    void setGroundWeaponCooldown(byte groundWeaponCooldown);
    byte groundWeaponCooldown;
    int resourceContained;
    int getDistance(Unit *unit);
    int workersMiningFromThis;
    byte mainOrderState;
    std::string getStateDescription(void);
    Unit *getOrderTarget(void);
    void setOrderTarget(Unit *unit);

    byte getOrderID(void);
    void setOrderID(byte orderID);
  private :
    short int moveX, moveY;
    int hp, shield;
    byte hpFraction, shieldFraction;
    int acceleration;
    Unit *orderTarget;
    byte mainOrderID;
 };
